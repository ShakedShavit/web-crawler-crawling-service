const { doesKeyExistInRedis, getHashValuesFromRedis } = require("../utils/redis");
const {
    sendMessageToQueue,
...
const sendNewMessages = async (messageUrl, messageLevel, nextQueueUrl, links = []) => {
    try {
        for (let link of links)
            sendMessageToQueue(nextQueueUrl, link, messageLevel + 1, messageUrl).catch((err) => {
...
const crawl = async (crawlInfo) => {
    ...
    try {
        do {
            ...
            if (!doesQueueHashExist || doesQueueHashExist === "false")
                throw new Error(`${crawlRedisHashKey} does not exist in redis`);

            // If other crawlers finished the scraping
            let [currQueueUrl, nextQueueUrl, isCrawlingDone] = await getHashValuesFromRedis(
                crawlRedisHashKey,
                [redisHashFields[2], redisHashFields[3], redisHashFields[0]]
            );
            if (isCrawlingDone === "true") break;

            let messages;
            try {
                messages = await pollMessagesFromQueue(currQueueUrl, 10);
            } catch (error) {
            ...
            for (let message of messages) {
                ...
                getLinksAndAddPageToTree({ messageUrl, messageLevel, parentUrl }, crawlInfo).then(
                    (links) => {
                        if (links.length !== 0) {
                            if (!!nextQueueUrl)
                                sendNewMessages(
