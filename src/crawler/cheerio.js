...
const getPageInfo = async (url) => {
    ...
    try {
        ...
        $("a").each((index, element) => {
            ...
            if (hrefVal[0] === "/") {
                // This means that the path afterwards is added to the core site's url (instead of the current path)
                let endOfCoreSiteUrlIndex = url.indexOf("/", 8);
                let modifiedUrl = url;
                if (endOfCoreSiteUrlIndex !== -1) {
                    // If the parent url does end with a - /
